...
function ContactCard({ heading, subheading, person, itemTitle, showCollectBtn, collected, collecting, onCollect }) {
  ...
  const subject = enc(`Campus Lost & Found — ${itemTitle}`);
  const body    = enc(`Hi ${person.name}, regarding "${itemTitle}" on Campus Lost & Found — the claim has been approved. Let's arrange the handover.`);
  const waMsg   = enc(`Hi ${person.name}! The claim for "${itemTitle}" on Campus Lost & Found has been approved. Let's meet to hand it over.`);

  const contactBtns = [
    { href: `tel:${digits}`,                                      bg: "#2563eb", label: "Call",      Icon: CallIcon },
    { href: `mailto:${person.email}?subject=${subject}&body=${body}`, bg: "#7c3aed", label: "Email", Icon: MailIcon },
    { href: `https://wa.me/${digits}?text=${waMsg}`,              bg: "#25D366", label: "WhatsApp",  Icon: WAIcon  },
  ];
    ...
    <div style={{ borderRadius: 16, overflow: "hidden", border: "2px solid #22c55e", boxShadow: "0 4px 24px rgba(34,197,94,0.18)", marginBottom: 24 }}>
      ...
      <div style={{ padding: "22px", background: "#f0fdf4" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#16a34a", textTransform: "uppercase", letterSpacing: "0.7px", marginBottom: 5 }}>
            Contact Person
          </div>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
          {contactBtns.map(({ href, bg, label, Icon }) => (
            <a key={label} href={href} target="_blank" rel="noreferrer"
              style={{ flex: 1, minWidth: 100, display: "flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "11px 14px", borderRadius: 12, background: bg, color: "#fff", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
              <Icon /> {label}
            </a>
          ))}
        </div>
        ...
        {showCollectBtn && !collected && (
          ...
          >
            {collecting
              ...
              : <><CheckIcon /> I Have Collected My Item — Mark as Resolved</>
            }
          </button>
        )}
        ...
        {collected && (
          <div style={{ background: "#dcfce7", borderRadius: 12, padding: "14px 18px", display: "flex", alignItems: "center", gap: 10, border: "1px solid #86efac" }}>
            <CheckIcon />
            <span style={{ fontWeight: 700, color: "#15803d" }}>Item successfully collected and marked as resolved!</span>
          </div>
        )}
      </div>
    </div>
...
export function ItemDetailPage({ onToast }) {
  ...
  useEffect(() => {
    ...
    fetchItem(id)
      .then(async (fetched) => {
        setItem(fetched);
        const needsApproved = fetched.status === "claim_approved" || fetched.status === "resolved";
        if (isAdmin) {
          const claims = await fetchClaims(id);
          setAdminClaims(Array.isArray(claims) ? claims : []);
        } else if (needsApproved) {
  ...
  /* ── Role detection ──────────────────────────────────────────────────── */
  // poster: the one who created the item post
  const isPoster   = user && item && emailEq(user.email, item.postedBy?.email);
  // claimer: the one whose claim was approved
  ...
  /* ── Who clicks "I've collected"?
       LOST item:  the poster (they lost it, they collect it from the finder/claimer)
       FOUND item: the claimer (they're the true owner, they collect from the finder/poster)
  ────────────────────────────────────────────────────────────────────────── */
  ...
  const handleCollect = async () => {
    ...
    try {
      ...
      onToast(err.message || "Failed to mark collected.", "error");
    } finally {
      setCollecting(false);
    }
  };
  ...
  }[item.status] ?? { label: item.status || "Open", bg: "#fef3c7", color: "#b45309", border: "#fcd34d" };
    ...
    <main className="page--narrow">
      ...
      <div className="card" style={{ overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", height: 300 }}>
          {item.image
            ...
            : <div style={{ height: "100%", background: item.type === "found" ? "linear-gradient(135deg,#dcfce7,#86efac)" : "linear-gradient(135deg,#fef9c3,#fde047)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <svg width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="rgba(0,0,0,0.15)" strokeWidth="1">
        ...
        <div style={{ padding: "28px 32px" }}>
          ...
          {/* ═══════════════════════════════════════════════════════════════
              POSTER — sees claimer's contact info after approval
          ═══════════════════════════════════════════════════════════════ */}
          {isPoster && approved && approvedClaim && (
            <ContactCard
              heading={item.type === "lost" ? "Someone found your item!" : "Claim on your found item approved!"}
          ...
          {/* ═══════════════════════════════════════════════════════════════
              CLAIMER — sees poster's contact info after approval
          ═══════════════════════════════════════════════════════════════ */}
          {isClaimer && !isPoster && approved && approvedClaim && (
            <ContactCard
              heading={item.type === "lost" ? "Your finder report was approved!" : "Your ownership claim was approved!"}
          ...
          {/* ═══════════════════════════════════════════════════════════════
              EVERYONE ELSE (not poster, not approved claimer, not admin)
          ═══════════════════════════════════════════════════════════════ */}
            ...
            <>
              ...
              <div style={{ borderRadius: 14, padding: "18px 22px", marginBottom: 24, background: "var(--gray-50)", border: "1.5px solid var(--border)" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 13, color: "var(--text-muted)" }}>
                  <LockIcon />
                  Contact details are hidden — revealed only after admin verifies a claim
                </div>
              </div>
              ...
              {(!item.status || item.status === "open") && (
                user ? (
                  ...
                  <div style={{ borderRadius: 14, padding: "20px 22px", background: "var(--primary-bg)", border: "1.5px solid var(--primary-border)", textAlign: "center" }}>
                    ...
                    <p style={{ color: "var(--text-sub)", fontSize: 14, marginBottom: 16 }}>
                      You need an account to submit a claim. Your contact details from your profile will be used.
                    </p>
              ...
              {item.status === "claim_approved" && (
                <div className="info-box info-box-success" style={{ textAlign: "center", fontWeight: 600 }}>
                  A claim has been approved. The two parties are arranging the handover.
                </div>
              )}
          ...
          {/* ═══════════════════════════════════════════════════════════════
              ADMIN — full poster info + all claims
          ═══════════════════════════════════════════════════════════════ */}
          {isAdmin && (
            <>
              {/* Poster full contact */}
              ...
              {adminClaims.length === 0
                ? <p style={{ color: "var(--text-muted)", textAlign: "center", padding: "20px 0" }}>No claims submitted yet.</p>
                : adminClaims.map((c) => (
                  <div key={c.id} style={{
