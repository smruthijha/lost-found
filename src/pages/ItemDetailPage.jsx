...
const PhoneIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.41 2 2 0 0 1 3.6 1.26h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 8.9a16 16 0 0 0 6 6l.91-.91a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>
  </svg>
);
const MailIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
function ContactCard({ heading, subheading, person, itemTitle, showCollectBtn, collected, collecting, onCollect }) {
  ...
  const subject = encodeURIComponent(`Campus Lost & Found — ${itemTitle}`);
  const body    = encodeURIComponent(`Hi ${person.name}, this is regarding the item "${itemTitle}" on Campus Lost & Found. The claim has been approved — let's arrange the handover.`);
  const waText  = encodeURIComponent(`Hi ${person.name}! Your claim for "${itemTitle}" on Campus Lost & Found has been approved. Let's arrange to meet.`);
    ...
    <div style={{ borderRadius: 16, overflow: "hidden", border: "2px solid #22c55e", boxShadow: "0 4px 24px rgba(34,197,94,0.18)", marginBottom: 24 }}>
      ...
      <div style={{ padding: "22px", background: "#f0fdf4" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
          {/* Call */}
          <a href={`tel:${digits}`}
            style={{ flex: 1, minWidth: 120, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "12px 14px", borderRadius: 12, background: "#2563eb", color: "#fff", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
            <PhoneIcon /> Call
          </a>
          {/* Email */}
          <a href={`mailto:${person.email}?subject=${subject}&body=${body}`}
            style={{ flex: 1, minWidth: 120, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "12px 14px", borderRadius: 12, background: "#7c3aed", color: "#fff", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
            <MailIcon /> Email
          </a>
          {/* WhatsApp */}
          <a href={`https://wa.me/${digits}?text=${waText}`} target="_blank" rel="noreferrer"
            style={{ flex: 1, minWidth: 120, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "12px 14px", borderRadius: 12, background: "#25D366", color: "#fff", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
            <WhatsAppIcon /> WhatsApp
          </a>
        </div>

        {/* Their email + phone as readable text too */}
        <div style={{ background: "#fff", borderRadius: 10, padding: "12px 16px", border: "1px solid #bbf7d0", marginBottom: 16, fontSize: 14, lineHeight: 2 }}>
        ...
        {/* Mark collected button — only for the person who should confirm */}
        {showCollectBtn && !collected && (
          <button
            ...
            style={{
              ...
              padding: "14px", borderRadius: 12, border: "none", cursor: collecting ? "not-allowed" : "pointer",
        ...
        {collected && (
          <div style={{ background: "#dcfce7", borderRadius: 12, padding: "14px 18px", display: "flex", alignItems: "center", gap: 10, border: "1px solid #86efac" }}>
            ...
            <span style={{ fontWeight: 700, color: "#15803d" }}>Item has been marked as collected and resolved!</span>
          </div>
        )}
      </div>
    </div>
...
export function ItemDetailPage({ onToast }) {
    ...
    fetchItem(id)
      .then(async (fetched) => {
        ...
        if (isAdmin) {
          ...
          // Load approved claim so we can decide who sees whose info
  ...
  /* ── Role detection ─────────────────────────────────────────────────────
     isOwner  → the person who POSTED the item
     isClaimer → the person whose claim was APPROVED
  ────────────────────────────────────────────────────────────────────────── */
  const emailMatch = (a, b) =>
    a && b && a.trim().toLowerCase() === b.trim().toLowerCase();

  const isOwner   = user && item && emailMatch(user.email, item.postedBy?.email);
  const isClaimer = user && approvedClaim && emailMatch(user.email, approvedClaim.email);

  /* ── Who should see the "Mark as collected" button?
     For a LOST item:  the owner (poster) collects from the finder (claimer)
     For a FOUND item: the claimer (true owner) collects from the finder (poster)
  ────────────────────────────────────────────────────────────────────────── */
  const collectBelongsTo = item?.type === "lost" ? "owner" : "claimer";
  const showCollectBtn   =
    (collectBelongsTo === "owner"   && isOwner  ) ||
    (collectBelongsTo === "claimer" && isClaimer);
  ...
  const handleClaim = async (form) => {
    ...
    try {
      ...
      setShowModal(false);
      onToast("Claim submitted! Admin will verify it shortly.");
  ...
  const handleCollect = async () => {
    ...
    try {
      ...
      setApprovedClaim((p) => ({ ...p, collected: true }));
      onToast("Item marked as resolved. It has been removed from the public dashboard.");
    } catch (err) { onToast(err.message, "error"); }
  ...
  const approved   = item.status === "claim_approved" || item.status === "resolved";
  const statusMeta = {
  ...
  }[item.status] ?? { label: item.status, bg: "#f3f4f6", color: "#6b7280", border: "#e5e7eb" };
    ...
    <main className="page--narrow">
      ...
      <div className="card" style={{ overflow: "hidden" }}>
        ...
        <div style={{ padding: "28px 32px" }}>
          ...
          {/* ═══════════════════════════════════════════════════════════════
              SECTION A — POSTER sees CLAIMER'S contact info
              (after admin approves)
          ═══════════════════════════════════════════════════════════════ */}
          {isOwner && approved && approvedClaim && (
            <ContactCard
              heading={
                item.type === "lost"
                  ? "Someone found your item!"      // A lost it, B found it → A sees B's info
                  : "Claim on your found item approved!" // A found it, B claims it → A sees B's info
              }
              subheading="Admin verified this claim — contact them to arrange handover"
              person={{
                name:  approvedClaim.name,
          ...
          {/* ═══════════════════════════════════════════════════════════════
              SECTION B — CLAIMER sees POSTER'S contact info
              (after admin approves)
          ═══════════════════════════════════════════════════════════════ */}
          {isClaimer && !isOwner && approved && approvedClaim && (
            <ContactCard
              heading={
                item.type === "lost"
                  ? "Your finder report was approved!" // You found B's lost item
                  : "Your claim was approved!"         // You own this found item
              }
          ...
          {/* ═══════════════════════════════════════════════════════════════
              SECTION C — EVERYONE ELSE (not poster, not approved claimer)
          ═══════════════════════════════════════════════════════════════ */}
          {!isOwner && !isClaimer && !isAdmin && (
            <>
              {/* Poster name (no contact details) */}
              <div style={{ borderRadius: 14, padding: "18px 22px", marginBottom: 24, background: "var(--gray-50)", border: "1.5px solid var(--border)" }}>
                ...
                <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 7, fontSize: 13, color: "var(--text-muted)" }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                  Contact details are hidden — only revealed to verified parties
                </div>
              </div>

              {/* Claim button — only for open items */}
              {item.status === "open" && (
          ...
          {/* ═══════════════════════════════════════════════════════════════
              SECTION D — ADMIN sees everything
          ═══════════════════════════════════════════════════════════════ */}
          {isAdmin && (
            <>
              {/* Poster full contact */}
              ...
              {adminClaims.length === 0
                ...
                : adminClaims.map((c) => (
                  <div key={c.id} style={{
                    ...
                    background: c.status === "approved" ? "var(--success-bg)" : c.status === "rejected" ? "var(--danger-bg)" : "#fff",
                    border: `1.5px solid ${c.status === "approved" ? "var(--success-border)" : c.status === "rejected" ? "var(--danger-border)" : "var(--border)"}`,
                  }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, flexWrap: "wrap" }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 14, background: "rgba(0,0,0,0.04)", padding: "10px 14px", borderRadius: 8, lineHeight: 1.6, fontStyle: "italic" }}>
                          "{c.description}"
                        </div>
                        {c.collected && (
