...
import { fetchClaims, fetchApprovedClaim,
         markItemCollected }                  from "../firebase/claims";
...
/* ── Contact Card (shown to both parties once approved) ──────────────────── */
function ContactCard({ heading, subheading, person, itemTitle, showCollectBtn, collected, collecting, onCollect }) {
  if (!person?.name) return null;

  const digits = (person.phone || "").replace(/\D/g, "");
  const e      = encodeURIComponent;
  const waText = e(`Hi ${person.name}! Regarding "${itemTitle}" on Campus Lost & Found — the claim has been approved. Let's arrange the handover.`);
    ...
    <div style={{ borderRadius:16, overflow:"hidden", border:"2px solid #22c55e", boxShadow:"0 4px 24px rgba(34,197,94,0.18)", marginBottom:24 }}>
      ...
      <div style={{ padding:"22px", background:"#f0fdf4" }}>
        ...
        <div style={{ display:"flex", gap:10, marginBottom:14, flexWrap:"wrap" }}>
          <a href={`tel:${digits}`}
            style={{ flex:1, minWidth:90, display:"flex", alignItems:"center", justifyContent:"center", gap:7, padding:"12px 14px", borderRadius:12, background:"#2563eb", color:"#fff", fontWeight:700, fontSize:14, textDecoration:"none" }}>
            <CallIcon /> Call
          </a>
          <a href={`mailto:${person.email}?subject=${e(`Campus Lost & Found — ${itemTitle}`)}`}
            style={{ flex:1, minWidth:90, display:"flex", alignItems:"center", justifyContent:"center", gap:7, padding:"12px 14px", borderRadius:12, background:"#7c3aed", color:"#fff", fontWeight:700, fontSize:14, textDecoration:"none" }}>
            <MailIcon /> Email
          </a>
          <a href={`https://wa.me/${digits}?text=${waText}`} target="_blank" rel="noreferrer"
            style={{ flex:1, minWidth:90, display:"flex", alignItems:"center", justifyContent:"center", gap:7, padding:"12px 14px", borderRadius:12, background:"#25D366", color:"#fff", fontWeight:700, fontSize:14, textDecoration:"none" }}>
            <WAIcon /> WhatsApp
          </a>
        </div>
        ...
        {collected && (
          <div style={{ background:"#dcfce7", borderRadius:12, padding:"14px 18px", display:"flex", alignItems:"center", gap:10, border:"1px solid #86efac" }}>
            <CheckIcon />
            <span style={{ fontWeight:700, color:"#15803d" }}>Item collected and fully resolved!</span>
          </div>
        )}
      </div>
    </div>
...
export function ItemDetailPage({ onToast }) {
  ...
  const [claimLoading, setClaimLoad]   = useState(false);
  const [collecting,   setCollecting]  = useState(false);
  const [pageLoading,  setPageLoad]    = useState(true);

  /* ─── Load everything fresh from Firestore every time ─────────────────
     This is the KEY fix — we never rely on stale context state.
     We always re-fetch the item AND the approved claim directly.
  ──────────────────────────────────────────────────────────────────────── */
  const loadPage = async () => {
    ...
    try {
      // 1. Always get fresh item from Firestore
      const fetched = await fetchItem(id);
      setItem(fetched);
      console.log("ItemDetailPage loaded item:", fetched);

      // 2. Admin: load all claims
      if (isAdmin) {
        const claims = await fetchClaims(id);
        setAdminClaims(claims);
      }

      // 3. ALWAYS try to load approved claim when status is claim_approved or resolved
      //    This runs for EVERYONE (not just admin) so poster & claimer both get data
      if (fetched.status === "claim_approved" || fetched.status === "resolved") {
        const approved = await fetchApprovedClaim(id);
        console.log("ItemDetailPage approved claim:", approved);
        setApproved(approved);
      }
    } catch (e) {
      console.error("ItemDetailPage loadPage error:", e);
    } finally {
      setPageLoad(false);
    }
  };
  ...
  /* ─── Role flags ──────────────────────────────────────────────────────
     isPoster  → created the item post
     isClaimer → their claim was approved
  ──────────────────────────────────────────────────────────────────────── */
  ...
  /* ─── Who gets "Mark as Collected" button?
     LOST  item → the poster (they were missing it, they receive it from finder)
     FOUND item → the claimer (they are the true owner, they pick it up)
  ────────────────────────────────────────────────────────────────────── */
  ...
  const handleCollect = async () => {
    ...
    setCollecting(true);
    try {
      // Call Firestore directly — don't go through context
      await markItemCollected(id, approvedClaim.id);
      setItem((p)    => ({ ...p, status: "resolved" }));
      setApproved((p)=> ({ ...p, collected: true }));
      onToast("Item marked as resolved! Removed from public dashboard.");
    } catch (err) {
      ...
      <div className="card" style={{ overflow:"hidden" }}>
        ...
        <div style={{ padding:"28px 32px" }}>
          ...
          {isPoster && isApproved && approvedClaim && (
            <ContactCard
              heading={item.type === "lost" ? "Someone found your item!" : "Claim on your posted item approved!"}
          ...
          {/* ═══════════════════════════════════════════════════════════════
              POSTER but no approvedClaim loaded yet — warn them
          ═══════════════════════════════════════════════════════════════ */}
          {isPoster && isApproved && !approvedClaim && (
            <div className="info-box info-box-warning" style={{ marginBottom:24 }}>
              A claim has been approved. Contact details are loading — please refresh the page if they don't appear.
            </div>
          )}
          ...
          {/* ═══════════════════════════════════════════════════════════════
              PUBLIC: not poster, not claimer, not admin
          ═══════════════════════════════════════════════════════════════ */}
            ...
            <>
              <div style={{ borderRadius:14, padding:"18px 22px", marginBottom:24, background:"var(--gray-50)", border:"1.5px solid var(--border)" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:7, fontSize:13, color:"var(--text-muted)" }}>
                  <LockIcon /> Contact details hidden — revealed only to verified parties
                </div>
              </div>
          ...
          {/* ═══════════════════════════════════════════════════════════════
              ADMIN: full poster info + all claims with status
          ═══════════════════════════════════════════════════════════════ */}
          {isAdmin && (
            <>
              <div style={{ borderRadius:14, padding:"18px 22px", marginBottom:24, background:"var(--success-bg)", border:"1.5px solid var(--success-border)" }}>
                ...
                <div style={{ fontSize:14, lineHeight:2.2 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <MailIcon /> <a href={`mailto:${item.postedBy?.email}`} style={{ color:"var(--primary)", fontWeight:600 }}>{item.postedBy?.email}</a>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <CallIcon /> <a href={`tel:${item.postedBy?.phone}`} style={{ color:"var(--primary)", fontWeight:600 }}>{item.postedBy?.phone}</a>
                  </div>
                </div>
              </div>
